        ...
        for (int i = 2; i < trabajadores.length; i++) {
            if (i%2==0) {
               ...
               // public Programador(boolean experienciaPOO, StringBuilder lenguajes, String nombre, String NIF, String departamento, double sueldoBase, Fecha fecha) {
                trabajadores[i]= new Programador(true, sb, "program"+i, "123412334".concat(Integer.toString(i)), "prog", Math.random()*501+1000, new Fecha());
            } else {
                //(int personas, String nombre, String NIF, String departamento, double sueldoBase, Fecha fecha) 
                trabajadores[i]= new JefeProyecto(i+2, "jefe"+i, "1234124".concat(Integer.toString(i)), "prog", Math.random()*501+1500, new Fecha());
            }
        }
    }
    ...
    public static Trabajador crearTrabajador() {
        ...
        if (op.equals("PROGRAMADOR") || op.equals("JEFE DE PROYECTO")) {
            ...
            if (op.equals("PROGRAMADOR")) {
                ...
                trabajador = new Programador(poo, lenguajes, nombre, nif,departamento, sueldo,new Fecha(dia, mes, anyo));
            } else {
                System.out.print("¿Cuantas personas dirige?: ");
                int numP = sc.nextInt();
                trabajador = new JefeProyecto(numP, nombre,nif,  departamento, sueldo,new Fecha(dia, mes, anyo));
    ...
    public boolean actualizarPersonas(String nombre, int personas) {
        ...
        for (int i = 0; i < trabajadores.length && !todoOk; i++) {
            ...
            if (trabajadores[i] != null && trabajadores[i].getClass().getSimpleName().equals("JefeProyecto") && trabajadores[i].nombre.equalsIgnoreCase(nombre)) {
                //((JefeProyecto) trabajadores[i]).personas = personas;
                JefeProyecto aux = (JefeProyecto)trabajadores[i];
                aux.personas = personas;
                todoOk = true;
            }
        }
        return todoOk;
